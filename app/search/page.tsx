...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <main className="search-page">
    ...
    <section className="search-results" aria-live="polite"><p>{query ? `${resultCount} results for “${q}”` : "Browse everything"}</p>
      ...
      {garments.length > 0 && <div className="search-group"><h2>Garments</h2><div>{garments.map(([id, product]) => <Link href="/customise" key={id}><Image src={product.image} alt="" width={164} height={160}/><span><b>{product.name}</b><small>Customise colours, style, fit and size</small></span><strong>From £{product.amount / 100}</strong></Link>)}</div></div>}
