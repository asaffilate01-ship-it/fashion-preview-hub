...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <main className="search-page">
    <header className="journal-header"><Link className="journal-brand" href="/">KALËTHON</Link><nav><Link href="/customise">Customise</Link><Link href="/journal">Journal</Link><BagLink /></nav></header>
    ...
    <section className="search-results" aria-live="polite"><p>{query ? `${resultCount} results for “${q}”` : "Browse everything"}</p>
      {resultCount === 0 && <div className="search-empty"><h2>No exact match yet.</h2><p>Try a garment, sport, city or fabric term—or browse the custom studio.</p><Link href="/customise">Customise a garment ↗</Link></div>}
      {garments.length > 0 && <div className="search-group"><h2>Garments</h2><div>{garments.map(([id, product]) => <Link href="/customise" key={id}><img src={product.image} alt=""/><span><b>{product.name}</b><small>Customise colours, style, fit and size</small></span><strong>From £{product.amount / 100}</strong></Link>)}</div></div>}
      {sports.length > 0 && <div className="search-group"><h2>Sports</h2><div>{sports.map((sport) => <Link href={`/sport/${sport.slug}`} key={sport.slug}><img src={sport.image} alt=""/><span><b>{sport.name} clothing</b><small>{sport.eyebrow}</small></span><strong>Explore ↗</strong></Link>)}</div></div>}
      {stories.length > 0 && <div className="search-group"><h2>Journal</h2><div>{stories.map((article) => <Link href={`/journal/${article.slug}`} key={article.slug}><img src={article.image} alt=""/><span><b>{article.title}</b><small>{article.category} · {article.readTime}</small></span><strong>Read ↗</strong></Link>)}</div></div>}
    </section>
  </main>;
}
