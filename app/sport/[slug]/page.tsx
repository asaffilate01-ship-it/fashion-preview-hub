...
export default async function SportPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="sport-page">
    <header className="journal-header"><Link className="journal-brand" href="/">KALËTHON</Link><nav><Link href="/#design-yours">Customise</Link><Link href="/measurements">Measurements</Link><Link href="/journal">Journal</Link></nav></header>
    ...
    <section className="sport-products" id="sport-products">
      ...
      <div className="sport-product-grid">{collection.products.map((product, index) => <article key={product.name}><span>0{index + 1} / {product.category}</span><div><h3>{product.name}</h3><p>{product.detail}</p></div><strong>{product.price}</strong><Link href="/#design-yours">Customise <b>↗</b></Link></article>)}</div>
    </section>
